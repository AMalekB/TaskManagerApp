...
export function containsSpecialCharacters(str) {
    // Regex pour détecter les caractères spéciaux dangereux (sauf les accents, points, virgules et espaces)
    const specialCharsRegex = /[!@#$%^&*()_+\-=\[\]{};':"\\|<>\/?]/;
    return specialCharsRegex.test(str);
}

// Fonction pour vérifier les espaces multiples
export function hasMultipleSpaces(str) {
    return /\s{2,}/.test(str);
}
...
export function validatePriority(priority) {
    if (!priority || isNaN(parseInt(priority))) {
        return { isValid: false, message: "Veuillez sélectionner une priorité valide" };
    }
